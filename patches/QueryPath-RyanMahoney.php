...
final class QueryPathImpl implements QueryPath, IteratorAggregate
{
    ...
    private $options = [];
    private $matches = [];
    private $last = []; // Last set of matches.
    ...
    public function __construct($document = NULL, $string = NULL, $options = [])
    {
        ...
        elseif (is_object($document)) {
            if ($document instanceof QueryPath) {
                ...
                $this->matches = [$document->documentElement];
            ...
            $this->matches = [$this->document->documentElement];
        } else {
    ...
    {
        $this->setMatches([$this->matches[$index]]);
    ...
    public function writeHTML($headers = [])
    {
