...
class DOMQueryTest extends TestCase
{
	...
	public function testIgnoreParserWarnings()
	{
		...
		Options::merge(['ignore_parser_warnings' => true]);
		...
		Options::set([]); // Reset to empty options.
	}
	...
	public function testReplaceEntitiesOption()
	{
		...
		Options::set(['replace_entities' => true]);
		$this->assertTrue(strpos($xml, '<foo>&amp;</foo>') !== false);
		Options::set([]);
	}
	...
	public function testFilterLambda()
	{
		if (version_compare(PHP_VERSION, '8.0.0', '>=')) {
			$this->markTestSkipped('create_function removed in PHP8.0');
		}
	...
	public function testAppend()
	{
		...
		$this->assertEquals(4, qp($file, 'inner')->append(qp(
	...
	public function testPrepend()
	{
		...
		$this->assertEquals(4, qp($file, 'inner')->prepend(qp(
			'<?xml version="1.0"?><root><test/><test/></root>',
			'test'
		))->top()->find('test')->count());
	}
	...
	public function testBefore()
	{
		...
		$this->assertEquals(4, qp($file, 'inner')->before(qp(
			'<?xml version="1.0"?><root><test/><test/></root>',
			'test'
		))->top()->find('test')->count());
	}
	...
	public function testAfter()
	{
		...
		$this->assertEquals(4, qp($file, 'inner')->after(qp(
			'<?xml version="1.0"?><root><test/><test/></root>',
			'test'
		))->top()->find('test')->count());
	}
	...
	public function testWrap()
	{
		...
		$xml = qp(
			...
			'<?xml version="1.0"?><root><test id="testWrap"></test><test id="ignored"></test></root>',
			'test'
		))->get(0)->ownerDocument->saveXML();
		$this->assertEquals(1, qp($xml, '#testWrap')->get(0)->childNodes->length);
		...
		$xml = qp(
			$file,
			'li'
		)->wrap('<test class="testWrap"><inside><center/></inside></test>')->get(0)->ownerDocument->saveXML();
	...
	public function testWrapInner()
	{
		...
		$qp = qp(
			...
			'<?xml version="1.0"?><root><test class="testWrap"/><test class="ignored"/></root>',
			'test'
		));
	...
	public function testRemoveClass()
	{
		...
		$this->assertTrue(qp(
			$file,
			'#inner-one'
		)->addClass('testClass')->removeClass('innerClass')->hasClass('testClass'));
	}
